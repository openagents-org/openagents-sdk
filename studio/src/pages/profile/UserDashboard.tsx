...
const UserDashboard: React.FC = () => {
  ...
  const enabledPluginKeys = React.useMemo(() => {
    ...
    enabledModules.forEach((moduleName) => {
      ...
      if (pluginKey) {
        ...
        console.log(`⚠️ UserDashboard: Module "${moduleName}" not found in MODULE_PLUGIN_MAP`);
      }
    });
  ...
  const primaryRoutes = React.useMemo(() => {
    ...
    console.log('📊 UserDashboard primary routes:', routes.map(r => r.navigationConfig?.key));
    return routes;
  }, [enabledPluginKeys]);
  ...
  const secondaryRoutes = React.useMemo(() => {
    const routes = dynamicRouteConfig
      ...
      .sort((a, b) => (a.navigationConfig?.order || 0) - (b.navigationConfig?.order || 0));
    console.log('📊 UserDashboard secondary routes:', routes.map(r => r.navigationConfig?.key));
    return routes;
  }, [enabledPluginKeys]);
  ...
  const getModuleColorScheme = (index: number) => {
    const colors = [
      { bg: "bg-blue-100 dark:bg-blue-900", icon: "text-blue-600 dark:text-blue-400" },
      { bg: "bg-indigo-100 dark:bg-indigo-900", icon: "text-indigo-600 dark:text-indigo-400" },
      { bg: "bg-purple-100 dark:bg-purple-900", icon: "text-purple-600 dark:text-purple-400" },
      { bg: "bg-pink-100 dark:bg-pink-900", icon: "text-pink-600 dark:text-pink-400" },
      { bg: "bg-red-100 dark:bg-red-900", icon: "text-red-600 dark:text-red-400" },
      { bg: "bg-orange-100 dark:bg-orange-900", icon: "text-orange-600 dark:text-orange-400" },
      { bg: "bg-amber-100 dark:bg-amber-900", icon: "text-amber-600 dark:text-amber-400" },
      { bg: "bg-yellow-100 dark:bg-yellow-900", icon: "text-yellow-600 dark:text-yellow-400" },
      { bg: "bg-lime-100 dark:bg-lime-900", icon: "text-lime-600 dark:text-lime-400" },
      { bg: "bg-green-100 dark:bg-green-900", icon: "text-green-600 dark:text-green-400" },
      { bg: "bg-emerald-100 dark:bg-emerald-900", icon: "text-emerald-600 dark:text-emerald-400" },
      { bg: "bg-teal-100 dark:bg-teal-900", icon: "text-teal-600 dark:text-teal-400" },
      { bg: "bg-cyan-100 dark:bg-cyan-900", icon: "text-cyan-600 dark:text-cyan-400" },
      { bg: "bg-sky-100 dark:bg-sky-900", icon: "text-sky-600 dark:text-sky-400" },
      { bg: "bg-violet-100 dark:bg-violet-900", icon: "text-violet-600 dark:text-violet-400" },
    ];
      ...
      <div className="p-4">
                ...
                return (
                  ...
                  >
                    <CardContent className="flex items-center space-x-3 p-4">
                      <div className={`w-10 h-10 rounded-full ${colorScheme.bg} flex items-center justify-center flex-shrink-0`}>
                        {IconComponent && (
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="text-sm font-medium">{getTranslatedLabel(navConfig.key)}</div>
                        <CardDescription className="text-xs mt-0.5">
                          {t(`dashboard.modules.${navConfig.key}.desc`, { defaultValue: '' })}
                        </CardDescription>
                      </div>
                    </CardContent>
                  </Card>
          ...
          <div className="mb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
              {secondaryRoutes.map((route, index) => {
                ...
                const IconComponent = getIconComponent(navConfig.icon);
                const colorScheme = getModuleColorScheme(primaryRoutes.length + index);
                const routePath = route.path.replace("/*", "");
        ...
        {primaryRoutes.length === 0 && secondaryRoutes.length === 0 && (
          <EmptyState
            ...
            title={t('dashboard.modules.noModulesAvailable', { defaultValue: '暂无可用模块' })}
            description={`启用的模块: ${enabledModules.length > 0 ? enabledModules.join(', ') : '无'}`}
          />
        )}
      </div>
    </ScrollArea>
