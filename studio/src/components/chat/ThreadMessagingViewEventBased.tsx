...
import { ThreadChannel, AgentInfo, ThreadMessage } from '../../types/events';
...
>(({
  ...
  const [replyingTo, setReplyingTo] = useState<{ messageId: string; text: string; author: string } | null>(null);
  const [quotingMessage, setQuotingMessage] = useState<{ messageId: string; text: string; author: string } | null>(null);

  // Refs
  const messagesEndRef = useRef<HTMLDivElement>(null);
  const messagesRef = useRef<ThreadMessage[]>([]);
  const readMessageStore = useRef(new ReadMessageStore(
  ...
  const handleChannelSelect = useCallback(async (channelName: string) => {
    ...
    try {
      ...
      readMessageStore.current?.markChannelAsRead(channelName, messages.map(m => m.message_id));
    } catch (error) {
  ...
  const handleDirectMessageSelect = useCallback(async (agentId: string) => {
    ...
    try {
      ...
      readMessageStore.current?.markDirectMessageAsRead(agentId, messages.map(m => m.message_id));
    } catch (error) {
  ...
  const handleSendMessage = useCallback(async (content: string, replyToId?: string) => {
    ...
    console.log('🔧 HandleSendMessage called:', { content, replyToId, currentChannel, currentDirectMessage });
    ...
    const optimisticMessage: ThreadMessage = {
      ...
      message_type: replyToId ? 'reply_message' : (currentChannel ? 'channel_message' : 'direct_message'),
    ...
    try {
      ...
      if (currentChannel) {
        ...
        const filteredMessages = (messagesRef.current || []).filter((msg: ThreadMessage) => msg.message_id !== optimisticMessage.message_id);
      ...
      if (result.success) {
        ...
        if (result.messageId) {
          const updatedMessages = (messagesRef.current || []).map((msg: ThreadMessage) => 
  ...
  }, [currentChannel, currentDirectMessage, sendingMessage, sendChannelMessage, sendDirectMessage, connectionStatus.agentId, agentName, setMessages]);
  ...
  // Handle reactions
  const handleReaction = useCallback(async (messageId: string, reactionType: string, action: 'add' | 'remove' = 'add') => {
    try {
      ...
      if (result.success) {
        console.log(`${action === 'add' ? '➕' : '➖'} Reaction ${reactionType} ${action}ed to message ${messageId}`);
      } else {
  ...
  }, [channels, agents, currentChannel, currentDirectMessage, onThreadStateChange]);
        ...
        {showDocuments ? (
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {messages.length === 0 ? (
                <div className="text-center text-gray-500 dark:text-gray-400 py-8">
                  {currentChannel ? `No messages in #${currentChannel} yet. Start the conversation!` : 
                   currentDirectMessage ? `No messages with ${currentDirectMessage} yet.` :
            ...
            {(currentChannel || currentDirectMessage) && (
              <ThreadMessageInput
                onSendMessage={(text: string, replyTo?: string, quotedMessageId?: string) => {
                  console.log('🔧 ThreadMessageInput onSendMessage called:', { text, replyTo, quotedMessageId, replyingTo, quotingMessage });
                  ...
                  if (replyTo) {
                    ...
                    handleSendMessage(`> ${quotingMessage.text}\n\n${text}`, quotedMessageId);
                    setQuotingMessage(null);
                  } else {
                    handleSendMessage(text);
                  }
                }}
                disabled={sendingMessage || connectionStatus.status !== 'connected'}
                placeholder={
                  sendingMessage ? 'Sending...' :
                  currentChannel ? `Message #${currentChannel}` :
                  currentDirectMessage ? `Message ${currentDirectMessage}` :
