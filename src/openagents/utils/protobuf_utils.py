        ...
        try:
            for key, value in obj.fields.items():
                ...
                if hasattr(value, 'struct_value') and value.struct_value:
                    ...
                    logger.debug(f"Unknown protobuf value type for key '{key}': {type(value)} = {value}")
            return result
...
def extract_text_from_protobuf_payload(payload: Any) -> str:
    ...
    if isinstance(payload_dict, dict):
        ...
        elif 'payload' in payload_dict and isinstance(payload_dict['payload'], dict) and 'text' in payload_dict['payload']:
            return payload_dict['payload']['text']
            
        # Try nested content -> text    
        elif 'content' in payload_dict and isinstance(payload_dict['content'], dict) and 'text' in payload_dict['content']:
