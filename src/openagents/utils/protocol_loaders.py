...
def load_network_protocols(protocol_configs: List[Dict[str, Any]]) -> Dict[str, BaseProtocol]:
    """Dynamically load and instantiate network-level protocols based on configuration.
    
    Args:
        protocol_configs: List of protocol configuration dictionaries.
                         Each should have 'name', 'enabled', and optional 'config' keys.
    
    Returns:
        Dict[str, BaseProtocol]: Dictionary mapping protocol names to protocol instances
    """
    protocols = {}
    ...
    for protocol_config in protocol_configs:
        ...
        if not enabled or not protocol_name:
            logger.debug(f"Skipping disabled or unnamed protocol: {protocol_config}")
            continue
            
        try:
            # Extract the module path for the network protocol
            # For example, from 'openagents.protocols.discovery.openconvert_discovery'
            # we get module_path = 'openagents.protocols.discovery.openconvert_discovery.protocol'
            
            module_path = f"{protocol_name}.protocol"
            
            # Try to load the protocol class name from the protocol_manifest.json
                ...
                if module_spec and module_spec.origin:
                    ...
                    if manifest_path.exists():
                        with open(manifest_path, 'r') as f:
                            ...
                            protocol_class_name = manifest_data.get("network_protocol_class")
                            logger.debug(f"Found network protocol class name in manifest: {protocol_class_name}")
            ...
            # Try to find the protocol class
            protocol_class = None
            
            # First, try using the class name from the manifest
            if protocol_class_name and hasattr(module, protocol_class_name):
                protocol_class = getattr(module, protocol_class_name)
                logger.debug(f"Using protocol class from manifest: {protocol_class_name}")
            else:
                ...
                class_name_candidates = [
                    f"{protocol_short_name.title().replace('_', '')}Protocol",  # e.g., OpenconvertDiscoveryProtocol
                    "Protocol",  # Generic name
                    f"{protocol_short_name.title().replace('_', '')}NetworkProtocol"  # e.g., OpenconvertDiscoveryNetworkProtocol
                ]
                
                for class_name in class_name_candidates:
                    if hasattr(module, class_name):
                        protocol_class = getattr(module, class_name)
                        logger.debug(f"Found protocol class using naming pattern: {class_name}")
                        break
                
                if protocol_class is None:
                    # If we couldn't find a class with the expected names, look for any class that inherits from BaseProtocol
                    for attr_name in dir(module):
                        attr = getattr(module, attr_name)
                        if isinstance(attr, type) and issubclass(attr, BaseProtocol) and attr != BaseProtocol:
                            protocol_class = attr
                            logger.debug(f"Found protocol class by inheritance: {attr_name}")
                            break
            
            if protocol_class is None:
                logger.error(f"Could not find a suitable protocol class in module {module_path}")
                continue
            
            # Instantiate the protocol
            protocol_instance = protocol_class(protocol_name)
            
            # Apply configuration if provided
            if config:
                protocol_instance.update_config(config)
            
            protocols[protocol_name] = protocol_instance
            logger.info(f"Successfully loaded network protocol: {protocol_class.__name__} for {protocol_name}")
            
        except ImportError as e:
            logger.error(f"Failed to import protocol module for {protocol_name}: {e}")
        except Exception as e:
            logger.error(f"Error loading network protocol for {protocol_name}: {e}")
...
def load_protocol_adapters(protocol_names: List[str]) -> List[BaseProtocolAdapter]:
    ...
    Args:
        protocol_names: List of protocol names to load adapters for.
                       Format should be 'openagents.protocols.{category}.{protocol_name}'
                       Example: 'openagents.protocols.communication.simple_messaging'
    ...
    for protocol_name in protocol_names:
        try:
            ...
            # For example, from 'openagents.protocols.communication.simple_messaging'
            # we get module_path = 'openagents.protocols.communication.simple_messaging.adapter'
