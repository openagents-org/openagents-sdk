...
class BasicTestNetworkMod(BaseMod):
    ...
    def shutdown(self) -> bool:
        ...
        """
        logger.info(f"Basic Test mod shutting down. Processed {self.event_count} events.")
        return True
    
    async def handle_register_agent(self, agent_id: str, metadata: Dict[str, Any]) -> bool:
        ...
        logger.info(f"Agent {agent_id} registered with Basic Test mod. Total agents: {len(self.registered_agents)}")
    ...
    async def handle_unregister_agent(self, agent_id: str) -> bool:
        ...
        logger.info(f"Agent {agent_id} unregistered from Basic Test mod. Remaining agents: {len(self.registered_agents)}")
        return True
    ...
    async def process_event(self, event: Event) -> Optional[EventResponse]:
        ...
        if self.log_all_events:
            logger.info(f"Processing event #{self.event_count}: {event.event_name} from {event.source_id}")
    ...
    def set_intercept_mode(self, intercept: bool) -> None:
        ...
        logger.info(f"Event interception mode: {'enabled' if intercept else 'disabled'}")
