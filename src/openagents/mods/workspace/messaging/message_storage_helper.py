...
class MessageStorageHelper:
    ...
    def cleanup_old_memory(self, message_history: Dict[str, Event], 
                          message_to_thread: Dict[str, str], 
                          threads: Dict[str, Any]) -> List[str]:
        """
                ...
                for msg_id in old_message_ids:
                    ...
                    if msg_id in message_to_thread:
                        ...
                        if thread_id in threads:
                            ...
                            if hasattr(thread, 'root_message_id') and msg_id == thread.root_message_id:
                                del threads[thread_id]
                
                logger.info(f"Cleaned up {len(old_message_ids)} old messages from memory")
    ...
    def cleanup_excess_messages(self, message_history: Dict[str, Event]) -> List[str]:
        ...
        try:
            ...
            excess_count = len(message_history) - int(self.config.max_memory_messages * 0.8)
            ...
            logger.info(f"Emergency cleanup: removed {excess_count} excess messages from memory")
    ...
    def archive_messages_by_date(self, message_ids: List[str], message_history: Dict[str, Event]):
        """
        ...
        try:
            ...
            for msg_id in message_ids:
                ...
                message_date = datetime.fromtimestamp(event.timestamp).date().isoformat()
                ...
                try:
                    ...
                    logger.warning(f"Failed to serialize message {msg_id} for archiving: {e}")
            ...
            for date_str, daily_messages in messages_by_date.items():
                ...
                if archive_file.exists():
                    ...
                    try:
                        ...
                        logger.warning(f"Could not load existing archive {archive_file}: {e}")
                ...
                logger.info(f"Archived {len(daily_messages)} messages to {date_str}.json.gz")
    ...
    def cleanup_expired_archives(self):
        ...
        try:
            ...
            cutoff_timestamp = time.time() - (self.config.archive_retention_days * 24 * 3600)
            ...
            for archive_file in archives_dir.glob("*.json.gz"):
                try:
                    ...
                    logger.warning(f"Could not process archive file {archive_file}: {e}")
    ...
    def _serialize_message_history(self, message_history: Dict[str, Event]) -> Dict[str, Any]:
