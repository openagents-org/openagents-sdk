...
class ForumAgentAdapter(BaseModAdapter):
    ...
    async def create_forum_topic(self, title: str, content: str) -> Optional[str]:
        ...
        if self.connector is None:
            logger.error(f"Cannot create topic: connector is None for agent {self.agent_id}")
            return None
    ...
    async def edit_forum_topic(self, topic_id: str, title: Optional[str] = None, content: Optional[str] = None) -> bool:
        ...
        """
        if self.connector is None:
            logger.error(f"Cannot edit topic: connector is None for agent {self.agent_id}")
            return False
    ...
    async def delete_forum_topic(self, topic_id: str) -> bool:
        ...
        """
        if self.connector is None:
            logger.error(f"Cannot delete topic: connector is None for agent {self.agent_id}")
            return False
    ...
    async def post_forum_topic_comment(self, topic_id: str, content: str, parent_comment_id: Optional[str] = None) -> Optional[str]:
        ...
        """
        if self.connector is None:
            logger.error(f"Cannot post comment: connector is None for agent {self.agent_id}")
            return None
    ...
    async def _cast_vote(self, target_type: str, target_id: str, vote_type: str) -> bool:
    ...
    async def list_forum_topics(self, limit: int = 50, offset: int = 0, sort_by: str = "recent") -> Optional[Dict[str, Any]]:
        ...
        """
        return await self._query_forum("list_topics", limit=limit, offset=offset, sort_by=sort_by)
    ...
    async def search_forum_topics(self, query: str, limit: int = 50, offset: int = 0) -> Optional[Dict[str, Any]]:
        ...
        """
        return await self._query_forum("search_topics", query=query, limit=limit, offset=offset)
    ...
    async def _query_forum(self, query_type: str, **kwargs) -> Optional[Dict[str, Any]]:
        ...
        """
        if self.connector is None:
            logger.error(f"Cannot query forum: connector is None for agent {self.agent_id}")
            return None
