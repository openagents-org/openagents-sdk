...
from openagents.config.globals import SYSTEM_EVENT_HEARTBEAT, SYSTEM_EVENT_REGISTER_AGENT, SYSTEM_EVENT_UNREGISTER_AGENT
...
class OpenAgentsGRPCServicer(agent_service_pb2_grpc.AgentServiceServicer):
    ...
    async def SendEvent(self, request, context):
        ...
        try:
            logger.debug(f"gRPC unified event: {request.event_name} from {request.source_id}")
            ...
            if response_data and isinstance(response_data, dict):
                ...
                try:
                    response_any = Any()
                    response_any.type_url = "type.googleapis.com/openagents.EventResponseData"
                    response_any.value = json.dumps(response_data, default=str).encode('utf-8')
                    protobuf_response_data = response_any
                    logger.debug(f"🔧 GRPC_TRANSPORT: Successfully serialized unified event response for {request.event_name}")
    ...
    def _extract_payload_from_protobuf(self, protobuf_payload):
        ...
        if protobuf_payload and protobuf_payload.value:
            try:
                ...
                if 'payload' in payload_dict and isinstance(payload_dict['payload'], str) and 'fields {' in payload_dict['payload']:
                ...
                elif 'payload' in payload_dict and isinstance(payload_dict['payload'], dict):
