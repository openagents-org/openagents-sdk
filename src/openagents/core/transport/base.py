...
class Transport(ABC):
    ...
    def __init__(self, transport_type: TransportType, config: Optional[Dict[str, Any]] = None, is_notifiable: bool = True):
        ...
        self.peer_connection_handlers: List[Callable[[str, ConnectionState], Awaitable[None]]] = []
    ...
    def register_event_handler(self, handler: Callable[[Event], Awaitable[EventResponse]]):
    ...
    def register_peer_connection_handler(self, handler: Callable[[str, ConnectionState], Awaitable[None]]):
    ...
    async def _notify_peer_connection_handlers(self, peer_id: str, state: ConnectionState):
