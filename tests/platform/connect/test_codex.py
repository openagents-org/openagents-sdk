"""
Platform connect tests for Codex agent.

Tests that `openagents start codex --create-workspace ...` can create a
...
Run:
    pytest tests/platform/connect/test_codex.py -v
"""
...
def agent_name():
    ...
    name = f"ci-codex-{uuid.uuid4().hex[:8]}"
    yield name
...
class TestCodexConnect:
    """Test connecting Codex to a workspace."""

    def test_agent_installed(self):
        """Codex must be installed before we can connect it."""
        assert shutil.which(BINARY_NAME) is not None, (
            f"'{BINARY_NAME}' not on PATH. "
            f"Run install tests first: pytest tests/platform/install/test_codex.py"
        )

    def test_create_workspace_and_connect(self, agent_name):
        """`openagents start codex --create-workspace` should create a
