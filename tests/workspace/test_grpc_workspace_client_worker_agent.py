...
class MockWorkerAgent(WorkerAgent):
    ...
    async def on_direct(self, msg: EventContext):
        ...
        print(f"🔥 DIRECT MESSAGE: Agent {self.client.agent_id} received from {msg.source_id}: {msg.text}")
    ...
    async def on_channel_post(self, msg: ChannelMessageContext):
        ...
        print(f"🔥 CHANNEL POST: Agent {self.client.agent_id} received in #{msg.channel} from {msg.source_id}: {msg.text}")
        ...
        if "test-reply-trigger" in msg.text.lower():
            ...
            if response.success and hasattr(response, 'data') and response.data:
                ...
                if reply_message_id:
                    self.sent_messages[reply_message_id] = f"Reply from {self.client.agent_id}"
    
    async def on_channel_reply(self, msg: ReplyMessageContext):
        """Handle channel replies and store them for verification."""
        print(f"🔥 CHANNEL REPLY: Agent {self.client.agent_id} received in #{msg.channel} from {msg.source_id}: {msg.text} (replying to {msg.reply_to_id})")
    ...
    async def on_channel_mention(self, msg: ChannelMessageContext):
        ...
        print(f"🔥 CHANNEL MENTION: Agent {self.client.agent_id} mentioned in #{msg.channel} by {msg.source_id}: {msg.text}")
    ...
    async def on_reaction(self, msg: ReactionContext):
        ...
        print(f"🔥 REACTION: Agent {self.client.agent_id} received reaction {msg.reaction_type} ({msg.action}) on message {msg.target_message_id} from {msg.reactor_id}")
...
async def test_workspace_to_worker_channel_message(workspace_client, worker_agent):
    ...
    if len(agent.received_channel_posts) > 0:
        ...
        print("✅ Message delivery working - event received by client (WorkerAgent event handlers need AgentRunner integration fix)")
...
async def test_workspace_to_worker_direct_message(workspace_client, worker_agent):
    ...
    print(f"📊 Worker agent received {len(agent.received_direct_messages)} direct messages")
...
async def test_workspace_mention_worker_agent(workspace_client, worker_agent):
    ...
    mention_message = f"Hey @{agent.client.agent_id}, this is a mention test! Time: {time.time()}"
    ...
    print(f"📊 Worker agent received {len(agent.received_channel_mentions)} mentions and {len(agent.received_channel_posts)} regular posts")
...
async def test_workspace_react_to_worker_message(workspace_client, worker_agent):
    ...
    if hasattr(general_channel, 'react_to_message') and message_id:
        ...
        print("⚠️ Message ID not available or reaction method not supported - test skipped")
...
async def test_comprehensive_message_flow(workspace_client, worker_agent):
    ...
    mention_response = await channel.post(f"Hey @{agent.client.agent_id}, comprehensive mention test!")
    ...
    reply_trigger_response = await channel.post("Please test-reply-trigger in comprehensive test")
    ...
    print("✅ Comprehensive message flow test completed (results may vary due to channel delivery issues)")
