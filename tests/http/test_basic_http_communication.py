...
async def test_basic_http_event_communication(http_client_a, http_client_b):
    ...
    test_event = Event(
        ...
        payload={"text": "Hello from HTTP client A!", "test_id": "basic_http_comm_test"},
    ...
    async def message_handler(event):
        print(f"📨 HTTP Client B received event: {event.event_name} from {event.source_id}")
    ...
    test_messages = [
        ...
        if (msg.source_id == "http-client-a" and 
            msg.payload and 
            msg.payload.get("test_id") == "basic_http_comm_test")
    ]
    
    assert len(test_messages) >= 1, f"HTTP Client B should have received test message. Got {len(client_b_messages)} total messages"
...
async def test_multiple_messages_http_communication(http_client_a, http_client_b):
    ...
    for i in range(15):  # Wait up to 15 seconds
        ...
        test_messages = [
            ...
            if (msg.source_id == "http-client-a" and 
                msg.payload and 
                msg.payload.get("test_id") == "multiple_http_comm_test")
        ]
        
        print(f"   After {i+1}s: Received {len(test_messages)}/{num_messages} HTTP test messages")
    ...
    assert len(test_messages) == num_messages, f"HTTP Client B should have received all {num_messages} test messages. Got {len(test_messages)} test messages out of {len(client_b_messages)} total messages"
...
async def test_list_agents_http_communication(http_client_a, http_client_b):
    ...
    try:
        ...
        assert len(agent_ids) >= 2, "Should have at least 2 agents (http-client-a and http-client-b)"
    ...
    try:
        ...
        # Both clients should be in the list
        assert "http-client-a" in agent_ids_b, "http-client-a should be in the agent list from B"
        assert "http-client-b" in agent_ids_b, "http-client-b should be in the agent list from B"
        assert len(agent_ids_b) >= 2, "Should have at least 2 agents (http-client-a and http-client-b)"
...
async def test_agent_unregister_list_agents_http(http_client_a, http_client_b):
    ...
    assert "http-client-a" in agent_ids_initial, "http-client-a should be in initial agent list"
    assert "http-client-b" in agent_ids_initial, "http-client-b should be in initial agent list"
    ...
    try:
        ...
        assert "http-client-a" in agent_ids_after, "http-client-a should still be in agent list"
        
        # Verify client B is no longer there
        assert "http-client-b" not in agent_ids_after, "http-client-b should NOT be in agent list after disconnect"
        ...
        print(f"✅ Agent list correctly updated: {len(agent_ids_initial)} → {len(agent_ids_after)} agents")
...
async def test_event_subscription_filtering_http(http_client_a, http_client_b):
    ...
    async def subscription_handler(event):
        print(f"📨 HTTP Client B received subscribed event: {event.event_name} from {event.source_id}")
        client_b_messages.append(event)
    ...
    subscription_response = await http_client_b.subscribe_events(["test.subscription.*", "allowed.event"])
    ...
    for i in range(15):
        ...
        print(f"   After {i+1}s: HTTP Client B received {len(client_b_messages)} events")
...
async def test_bidirectional_http_communication(http_client_a, http_client_b):
    ...
    a_received_from_b = [msg for msg in client_a_messages if msg.source_id == "http-client-b"]
    b_received_from_a = [msg for msg in client_b_messages if msg.source_id == "http-client-a"]
    
    assert len(a_received_from_b) >= 1, "HTTP Client A should receive message from Client B"
    assert len(b_received_from_a) >= 1, "HTTP Client B should receive message from Client A"
